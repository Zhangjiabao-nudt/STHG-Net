...
class NPZFileLoaderEAGCN:
    def __init__(self, data_root, seq_len, pre_len):
        ...
        middle =self.sst_mmap[0:365, 600:800, 400:600]
        middle = middle[:, self.max_mask]
        print(middle.shape, type(middle))
        self.adj_dir = self.gradient_data_dir / 'adj.npz'
        # if not self.adj_dir.exists():
        #     generate_static_adjacency_matrix(middle.T, file_path=self.adj_dir)
...
class SpatioTemporalDatasetEAGCN(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        window_long = self.mmap_data[actual_idx - self.time_delta[0]:actual_idx - self.time_delta[0] + self.pre_len, 600:800, 400:600]
        window_short = self.mmap_data[actual_idx - self.time_delta[1]:actual_idx - self.time_delta[1] + self.pre_len, 600:800, 400:600]
...
class SpatioTemporalDataModuleEAGCN(LightningDataModule):
    def __init__(
            self,
            data_root: str,
            batch_size: int = 1,
    ...
    def prepare_data(self):
        ...
        if not self.file_loader:
            self.file_loader = NPZFileLoaderEAGCN(
                self.data_root,
                self.seq_len,
                self.pre_len
            )
    ...
    def setup(self, stage: str = None):
        ...
        # print(test_list)
        if stage == 'test':
            self.test_dataset = SpatioTemporalDatasetEAGCN(
        ...
        self.train_dataset = SpatioTemporalDatasetEAGCN(
            mmap_data=self.file_loader.sst_mmap,
            start_idx=360,
            # end_idx=train_size,
            end_idx=total_days,
        ...
        self.val_dataset = SpatioTemporalDatasetEAGCN(
            mmap_data=self.file_loader.sst_mmap,
            start_idx=train_size,
            end_idx=total_days,
            seq_len=self.seq_len,
    ...
    def add_data_specific_arguments(parent_parser):
        ...
        parser.add_argument('--long_dim', type=int, default=360)
        parser.add_argument('--short_dim', type=int, default=30)
        return parser
...
if __name__ == "__main__":
    ...
    dm = SpatioTemporalDataModuleEAGCN(
