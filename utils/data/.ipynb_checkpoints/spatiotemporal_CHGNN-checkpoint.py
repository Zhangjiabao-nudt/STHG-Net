...
class CHGNNNPZFileLoader:
    def __init__(self, data_root, seq_len, pre_len):
        ...
        self.mask_dir = self.gradient_data_dir / 'gradient_long_mask_list.npz'
        if not self.mask_dir.exists():
            self.get_long_mask(self.mask_dir)
        self.mask_long_list = np.load(self.mask_dir)["mask"]
        print(type(self.mask_long_list), len(self.mask_long_list), self.mask_long_list.shape)
        ...
        # 创建超图中超边
        self.hyper_graph_path = self.data_root / "hypergraph_edges"
        if not self.hyper_graph_path.exists(): mkdir(self.hyper_graph_path)
        if is_path_empty(self.hyper_graph_path):
            # compute_and_save_graph(self.gradient, self.hyper_graph_path, self.valid_indices, self.time_stamps,
            #                        self.max_mask)
            compute_and_save_graph(self.gradient, self.hyper_graph_path, self.valid_indices, self.time_stamps,
                                   self.mask_list)
        self.hyper_edges_list = sorted([f for f in self.hyper_graph_path.glob("*.npy") if f.stem.isdigit()])
        if len(self.hyper_edges_list) == 0:
            raise ValueError("No hypergraph edges found.")

        self.hyper_adjacency_path = self.data_root / "hypergraph_adjacency"
        if not self.hyper_adjacency_path.exists(): mkdir(self.hyper_adjacency_path)
        if is_path_empty(self.hyper_adjacency_path):
            # compute_and_save_adjacency(self.sst_mmap, self.gradient, self.hyper_edges_list,
            #                            self.hyper_adjacency_path, self.valid_indices, self.time_stamps,
            #                            lens=self.seq_len + self.pre_len,
            #                            feat=self.feat, use_normalize=False, max_mask=self.max_mask)
            compute_and_save_adjacency(self.sst_mmap, self.gradient, self.hyper_edges_list,
                                       self.hyper_adjacency_path, self.valid_indices, self.time_stamps,
                                       lens=self.seq_len + self.pre_len,
                                       feat=None, use_normalize=False, max_mask=self.mask_list)
        self.adjacency_list = sorted([f for f in self.hyper_adjacency_path.glob("*.pth") if f.stem.isdigit()])
        # 创建超图中超边 long time
        self.hyper_graph_path = self.data_root / "half_hypergraph_edges"
        if not self.hyper_graph_path.exists(): mkdir(self.hyper_graph_path)
        if is_path_empty(self.hyper_graph_path):
            # compute_and_save_graph(self.gradient_long, self.hyper_graph_path, self.valid_indices_half, self.time_stamps,
            #                        self.max_mask)
            compute_and_save_graph(self.gradient_long, self.hyper_graph_path, self.valid_indices_half, self.time_stamps,
                                   self.mask_long_list)
        self.hyper_edges_list = sorted([f for f in self.hyper_graph_path.glob("*.npy") if f.stem.isdigit()])
        ...
        self.hyper_adjacency_path = self.data_root / "half_hypergraph_adjacency"
        if not self.hyper_adjacency_path.exists(): mkdir(self.hyper_adjacency_path)
        if is_path_empty(self.hyper_adjacency_path):
            # compute_and_save_adjacency(self.sst_mmap, self.gradient_long, self.hyper_edges_list,
            #                            self.hyper_adjacency_path, self.valid_indices_half, self.time_stamps,
            #                            lens=182,
            #                            feat=self.feat, use_normalize=False, max_mask=self.max_mask)
            compute_and_save_adjacency(self.sst_mmap, self.gradient_long, self.hyper_edges_list,
                                       self.hyper_adjacency_path, self.valid_indices_half, self.time_stamps,
                                       lens=182,
                                       feat=None, use_normalize=False, max_mask=self.mask_long_list)
        self.long_adjacency_list = sorted([f for f in self.hyper_adjacency_path.glob("*.pth") if f.stem.isdigit()])


        # for f in self.adjacency_list:
        #     a = torch.load(f, map_location='cpu')
        #     print(a.shape)
        # 验证存储是否一致
        # random_list = [random.randint(0, 1055) for _ in range(20)]
        # for num in random_list:
        #     print(self.hyper_edges_list[num], self.gradient_file_list[num], self.file_list[self.valid_indices[num]], self.adjacency_list[num])
    ...
    def _calculate_indices_half(self):
        return range(
            self.start_idx,
            self.end_idx, # 如果和时间数据一致，则大于等等于生成正常的，减去不影响  如果小于，则减去凑成正常的
            182,
        )
    ...
    def get_long_mask(self, file_path):
        mask_list = []
        for i, f in enumerate(tqdm(self.gradient_file_list, desc="得到mask list")):
            daily_gradient = self.gradient_long[i]
            daily_gradient = ~np.isnan(daily_gradient)
            daily_gradient = daily_gradient.reshape(1, daily_gradient.shape[0], daily_gradient.shape[1])
            mask_list.append(daily_gradient)
        mask_list = np.concatenate(mask_list)
...
class SpatioTemporalDataset(Dataset):
    def __init__(self, mmap_data, spatio_gradient, temporal_gradient, start_idx, end_idx, seq_len,
                 pre_len, device, adj_file_list, gradient_mmap, max_mask, move_step=10, normalize=2,
                 test_list=None, time_list=None, features=None):
        ...
        self.adj_file_list = adj_file_list
    ...
    def __getitem__(self, idx):
        ...
        # x = copy.deepcopy(window[:self.seq_len])
        # y = copy.deepcopy(window[self.seq_len:self.seq_len + self.pre_len])
        x = window[:self.seq_len]
        y = window[self.seq_len:self.seq_len + self.pre_len]
        # print(idx, actual_idx, actual_idx // 182, actual_idx // self.move_step)
        assert idx == ((actual_idx-self.valid_indices[0]) // self.move_step)
        return {
...
class SpatioTemporalDataMLoader(DataLoader):
...
class SpatioTemporalDataModuleCHGNN(LightningDataModule):
    def __init__(
            self,
            data_root: str,
            batch_size: int = 1,
    ...
    def prepare_data(self):
        ...
        if not self.file_loader:
            self.file_loader = CHGNNNPZFileLoader(
                self.data_root,
                self.seq_len,
                self.pre_len
            )
        # 数据大小
        self.adj = torch.load(self.file_loader.adjacency_list[0])
    ...
    def setup(self, stage: str = None):
        ...
        if stage == 'test':
            self.test_dataset = SpatioTemporalDataset(
                ...
                adj_file_list=self.file_loader.adjacency_list,
    ...
    def adjacency_matrix(self):
        return self.file_loader.adjacency_list, self.file_loader.long_adjacency_list
...
if __name__ == "__main__":
    ...
    dm = SpatioTemporalDataModuleCHGNN(
        # data_root=r"E:\PaperWork\SST",
        data_root=r"/home/jameszhang/桌面/SST/data_two",
        batch_size=1,
        seq_len=7,
        pre_len=1,
        move_step=10,
