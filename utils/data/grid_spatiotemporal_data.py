...
class GridSpatioTemporalDataset(Dataset):
    def __init__(self, mmap_data, start_idx, end_idx, seq_len, mask, batch_size,time_list,
                 pre_len, features, test_list=None, move_step=10, normalize=2):
...
class GridSpatioTemporalDataModule(LightningDataModule):
    ...
    def setup(self, stage: str = None):
        ...
        if stage == 'test':
            self.test_dataset = GridSpatioTemporalDataset(
                mmap_data=self.file_loader.sst_mmap,
                start_idx=total_days-20,
                end_idx=total_days,
