...
from utils.data.function import compute_mmap_statistics, compute_yearly_gradient, compute_daily_gradient, compute_and_save_graph, \
...
class NPZFileLoader:
    ...
    #     # 基于空间网格生成邻接矩阵
    #     lat = self.grid_info['spatial'][:, 0]
    #     lon = self.grid_info['spatial'][:, 1]
    #
    #     # 示例：生成基于距离的邻接矩阵
    #     adj = np.zeros((len(lat), len(lat)), dtype=np.float32)
    ...
    #             distance = 6371 * c  # 地球半径
    #
    #             if distance < 100:  # 100公里内视为相邻
    #                 adj[i, j] = 1.0 / (distance + 1e-5)
    #     return adj
...
class SpatioTemporalDataset(Dataset):
    def __init__(self, mmap_data, spatio_gradient, temporal_gradient, start_idx, end_idx, seq_len,time_list,
                 pre_len, features, device, adj_file_list, gradient_mmap, max_mask, move_step=10, normalize=2, test_list=None):
    ...
    def __getitem__(self, idx):
        ...
        mask = np.ones((200, 200), dtype=int)
        mask[np.any(window == 0, axis=0)] = 0
        # 标准化处理
        # 2. 数据标准化（在CPU进行）
        if self.normalize == 0:
            print('max initial')
...
class SpatioTemporalDataModule(LightningDataModule):
    ...
    def prepare_data(self):
        ...
        if not self.file_loader:
            self.file_loader = NPZFileLoader(
                self.data_root,
                self.seq_len,
                self.pre_len
            )
