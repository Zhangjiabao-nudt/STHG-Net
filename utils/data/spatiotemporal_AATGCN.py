...
from utils.data.function import compute_mmap_statistics, compute_yearly_gradient, compute_daily_gradient, \
    generate_adjacency_matrix_np
from utils.data.function_other_model import generate_static_adj
...
class NPZFileLoaderAATGCN:
    def __init__(self, data_root, seq_len, pre_len):
        ...
        """对每一个seq_len+pre_len应用对应的mask 目前只生成了30-7的"""
        ...
        """generate static adj, OVPGCN一样可以用于AATGCN"""
        ...
        print(len(self.static_adj_file_list))
...
class SpatioTemporalDatasetAATGCN(Dataset):
    def __init__(self, mmap_data, spatio_gradient, temporal_gradient, start_idx, end_idx, seq_len,
        ...
        self.time_delta = [long_dim, short_dim, 0]
    ...
    def __getitem__(self, idx):
        ...
        window = self.mmap_data[actual_idx - self.time_delta[2]:actual_idx + self.seq_len + self.pre_len, 600:800, 400:600]
        # window_long = self.mmap_data[actual_idx - self.time_delta[0]:actual_idx, 600:800, 400:600]
        # window_short = self.mmap_data[actual_idx - self.time_delta[1]:actual_idx, 600:800, 400:600]
...
class SpatioTemporalDataModuleAATGCN(LightningDataModule):
    def __init__(
            self,
            data_root: str,
            batch_size: int = 1,
    ...
    def prepare_data(self):
        ...
        if not self.file_loader:
            self.file_loader = NPZFileLoaderAATGCN(
                self.data_root,
                self.seq_len,
                self.pre_len
            )
        ...
        self.log_path = Path(self.data_root) / "log.pth"
        if not self.log_path.exists():
            a = torch.randn((1, 29021, 29021))
            log_u = torch.log(a)-torch.log(1-a)
            torch.save(log_u, self.log_path)
        self.log_u = torch.load(self.log_path)
    ...
    def setup(self, stage: str = None):
        ...
        # print(test_list)
        if stage == 'test':
            self.test_dataset = SpatioTemporalDatasetAATGCN(
        ...
        self.train_dataset = SpatioTemporalDatasetAATGCN(
            mmap_data=self.file_loader.sst_mmap,
            start_idx=360,
            # end_idx=train_size,
            end_idx=total_days,
        ...
        self.val_dataset = SpatioTemporalDatasetAATGCN(
            mmap_data=self.file_loader.sst_mmap,
            start_idx=train_size,
            end_idx=total_days,
            seq_len=self.seq_len,
    ...
    def adjacency_matrix(self):
        return self.file_loader.static_adj_file_list
...
if __name__ == "__main__":
    ...
    dm = SpatioTemporalDataModuleAATGCN(
