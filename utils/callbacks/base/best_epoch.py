...
class BestEpochCallback(Callback):
    ...
    def on_validation_batch_end(self, trainer, pl_module, outputs, batch, batch_idx):  #  dataloader_idx delete
        if (trainer.current_epoch + 1) % trainer.check_val_every_n_epoch != 0:
            return
        monitor_op = self.MONITOR_OP_DICT[self.mode]
        metrics_dict = copy.copy(trainer.callback_metrics)
        monitor_value = metrics_dict.get(self.monitor[1], self.best_value)  # 0 表示监视器中的train_loss
