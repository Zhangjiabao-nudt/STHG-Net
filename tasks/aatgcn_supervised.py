...
class AatgcnForecastTask(LightningModule):
    def __init__(
            self,
        ...
        super(AatgcnForecastTask, self).__init__()
        ...
        self.log_u = log
        self.mask = mask
        self.save_hyperparameters(ignore=['model', 'adj', 'log', 'mask', 'feat'])
    ...
    def forward(self, batch, training):
        ...
        for index, i in enumerate(current_index):
            ...
            x_batch = x_batch.view(batch_size, seq_len, num_nodes*num_nodes, 1).contiguous()
            predict = self.model(x_batch, adj[index], self.log_u, training)
        ...
        # # hidden = self.model(x, self.adj, self.log_u, training)
        # return hidden[:, ..., 0]
    ...
    def shared_step(self, batch, training):
        # (batch_size, seq_len/pre_len, num_nodes)
        # x, y, current_index = batch["x"], batch["y"], batch["current_index"]
        y = batch["y"]
        if self.log_u.device != y.device:
            # self.adj = self.adj.to(y.device)
            self.log_u = self.log_u.to(y.device)
        ...
        with autocast():  # 自动转换为 FP16
            predictions = self(batch, training)
    ...
    def training_step(self, batch):
        torch.cuda.empty_cache()
        predictions, y, mask = self.shared_step(batch, True)
    ...
    def test_step(self, batch):
        predictions, y = self.shared_step(batch, False)
    ...
    def validation_step(self, batch):
        predictions, y, mask = self.shared_step(batch, False)
        # mean std
        predictions = predictions * self.feat[3] + self.feat[2]
        y = y * self.feat[3] + self.feat[2]
        # min max
        ...
        for key, value in metrics.items():
            self.log(key, value, prog_bar=True, on_step=True, on_epoch=True, sync_dist=True, logger=True)
        self.log('rmpe', mape, prog_bar=True, on_step=True, on_epoch=True, sync_dist=True, logger=True)
